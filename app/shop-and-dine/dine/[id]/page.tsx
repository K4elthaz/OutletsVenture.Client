...
const dineDetails = () => {
  ...
  const [otherShops, setOtherShops] = useState<DineData[]>([]);

  const OnShopClickedEvent = async (shop: DineData) => {
    await update(dbRef(db, `${dinesReference}/${shop.id}`), {
  ...
  const recommendedShops = getRecommendedShops(dine, otherShops);
        ...
        <div className="container mx-auto px-4 sm:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {recommendedShops.length > 0
              ? recommendedShops.map((shop: DineData) => (
                  <div
                    key={shop.title}
